...
def drawCard(name, deck, hand):
  ...
  if len(deck) > 0: # protect against empty deck
    ...
    if cardRank in hand:
      ...
      if len(hand[cardRank]) == 4:  # This line has a bug. Change 'len(hand) == 4' to 'len(hand[cardRank]) == 4'
...
deck = [(x, y) for x in ranks for y in suits]  # Create a deck with 52 cards
hand = {}  # Initialize an empty hand

while len(deck) > 0:
    # Keep drawing card from the deck until no card left
    drawCard('Josh', deck, hand)
    print hand
